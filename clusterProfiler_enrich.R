...
"Usage: clusterProfiler_enrich.r  -i <file> -o <dir> --org <string> --orgdb <file> --orgdb_keggid <string> [--rlib <dir>]
...
if(!file.access(output_dir, 2) == 0){
    message("[Error]输出目录无写入权限:", output_dir)
    q()
}
...
# BP/CC/MF 绘图
############
go_bp_pdf          <- paste(output_dir, "go_bp.pdf", sep="/")
go_cc_pdf          <- paste(output_dir, "go_cc.pdf", sep="/")
go_mf_pdf          <- paste(output_dir, "go_mf.pdf", sep="/")
...
if(nrow(cc) >= 1) cc$Subontologies <- "CC"

bp <- as.data.frame(ego_bp)
if(nrow(bp) >= 1) bp$Subontologies <- "BP"

mf <- as.data.frame(ego_mf)
if(nrow(mf) >= 1) mf$Subontologies <- "MF"
go_enrichment   <- paste(output_dir, "enrichGO.txt", sep="/")
res <- rbind(bp, cc, mf)
...
############
ego <- res
types <- unique(ego$Subontologies)
DataForPlot <- list()
for(j in 1:length(types)){
    DataForPlot[[j]] <-  ego[grep(types[j],ego$Subontologies),]
    # 展示最显著的topN个
    TopN <- 10
...
# BP/MF/CC 颜色设定
color  <- colorRampPalette(c("Goldenrod1","Tomato1","MediumPurple4"))(length(unique(GO$Subontologies)))
color1 <- rep(color,as.numeric(table(GO$Subontologies)))
...
# 基因数量bar绘制
par(mar = c(5,0.1, 1, 3))  #  bottom, left, top, and right 边界宽度
...
legend("topright", legend=unique(GO$Subontologies),bty="n",fill=color,cex = 1.2)
y1 = par("usr")[3]
y2 = par("usr")[4]
...
# 防止访问过快导致数据下载失败
sleep_time = sample(30:60, 1, replace = FALSE)
...
if(orgdb_keggid == 'ENTREZID')  # ncbi-geneid 的情况下，我们把它转换为kegg-id
{
    ...
    # entrezid 转 keggid
    network_condition = 'ERROR'
    gene_map_entrezid2keggid = NULL
    round = 0
    while(network_condition == 'ERROR' || grepl("HTTP status was '403 Forbidden'", network_condition))
    {
        round = round + 1
        message("start circulation ", round)
        network_condition = tryCatch({gene_map_entrezid2keggid = suppressWarnings(bitr_kegg(gene_entrezid, 'ncbi-geneid', "kegg", org, drop = TRUE))
                                      'TRUE'},
                                      error = function(e){
                                                          message("get error ", conditionMessage(e))
                                                          'ERROR'},
                                      warning = function(w){
                                                          message("get warning ", conditionMessage(w))
                                                          conditionMessage(w)}
                                                          )

        message("get network_condition：", network_condition)
        message("get gene_map_entrezid2keggid：", head(gene_map_entrezid2keggid))
        sleep_time = sample(30:90, 1, replace = FALSE)
        if(network_condition == 'ERROR' || grepl("HTTP status was '403 Forbidden'", network_condition)) Sys.sleep(sleep_time)
    }
...
kk = NULL
round = 0
while(network_condition == 'ERROR' || network_condition == 'WARNING')
{
    round = round + 1
    message("start circulation ", round)
    network_condition = tryCatch({kk <- suppressWarnings(enrichKEGG(gene          = gene_map_symbol2keggid[, 'kegg'],
                                                                    organism      = org,
                                                                    keyType       = 'kegg',
                                                                    pvalueCutoff  = 1,
                                                                    qvalueCutoff  = 1,
    ...
    # 清空KEGG环境，防止网络问题导致误认为数据库下载成功
    pos <- 1
    envir <- as.environment(pos)
    assign(".KEGG_clusterProfiler_Env", new.env(), envir=envir)

    message("get network_condition：", network_condition)
    message("get kk：", head(as.data.frame(kk)))
    sleep_time = sample(30:90, 1, replace = FALSE)
    if(network_condition == 'ERROR' || network_condition == 'WARNING') Sys.sleep(sleep_time)
}
...
if(nrow(kk_enrich) > 0)
{
        ...
        p <-dotplot(kk, orderBy = "GeneRatio")
        print(p)
        dev.off()

#（2）表格输出
        kegg_enrichment <- paste(output_dir, "enrichKEGG.txt", sep="/")
